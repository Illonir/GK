...
int initGL(void)
{
    ...
    {   
      ...
      glBufferData(GL_ARRAY_BUFFER, sizeof(ver_rou), ver_rou, GL_STATIC_DRAW); 	//skopiowanie danych do pamieci aktywnego bufora
      ...
      glBufferData(GL_ARRAY_BUFFER, sizeof(col_rou), col_rou, GL_STATIC_DRAW);
      ...
      */
      ...
      //mod
      glBindBuffer(GL_ARRAY_BUFFER, vbo[2]);
      glBufferData(GL_ARRAY_BUFFER, sizeof(ver_box), ver_box, GL_STATIC_DRAW);
      glVertexAttribPointer(posAttrib, 3, GL_FLOAT, GL_FALSE, 0, 0);
      glEnableVertexAttribArray(posAttrib);

      glBindBuffer(GL_ARRAY_BUFFER, vbo[3]);
      glBufferData(GL_ARRAY_BUFFER, sizeof(col_box), col_box, GL_STATIC_DRAW);
      glVertexAttribPointer(colAttrib, 3, GL_FLOAT, GL_FALSE, 0, 0);
      glEnableVertexAttribArray(colAttrib);
...
int drawGLScene(int counter)
{
    ...
    glm::mat4 t = glm::translate(glm::mat4(), glm::vec3(0.0f, 1.5f, 0.0f));
    glm::mat4 tt = transformMatrix * t * s;
    GLint ttt = glGetUniformLocation(shaderProgram, "transformMatrix");  //pobranie polozenia macierzy bedacej zmienna jednorodna shadera
    glUniformMatrix4fv(ttt, 1, GL_FALSE, glm::value_ptr(tt)); //zapisanie macierzy bedacej zmienna jednorodna shadera wierzcholkow
